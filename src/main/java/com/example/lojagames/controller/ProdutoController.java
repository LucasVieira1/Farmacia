...
public class ProdutoController {
	...
	public ResponseEntity<List<Produto>> getByNome(@PathVariable String nome) { // path = passa oq vai ser passado no
																				// getmapping
	...
	public ResponseEntity<Produto> getById(@PathVariable Long id) {
		return produtoRepository.findById(id).map(resposta -> ResponseEntity.ok(resposta))
				.orElse(ResponseEntity.notFound().build()); // not found = erro, build = construir erro
	}
